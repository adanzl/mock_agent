...
bp = Blueprint("agnes", __name__)
logger = logging.getLogger(__name__)
_DOC_PATH = ROOT_DIR / "docs" / "agnes-api.md"
        ...
        jsonify(
            {
                "ok": False,
                "enabled": False,
                "error": "agnes disabled; set AGNES_ENABLED=1 in .env",
            }
        ),
        503,
    )
    ...
    return jsonify(
        {
            "ok": True,
            "service": "agnes",
            "enabled": bool(config.agnes_enabled),
        }
    )
...
def conversations():
    ...
    try:
        items = agnes_mgr.list_conversations(provider="agnes", limit=int(limit))
...
def conversation_detail(conversation_id: str):
    if not config.agnes_enabled:
        return _disabled_response()
    try:
        item = agnes_mgr.get_conversation(conversation_id)
        if item is None:
            return jsonify({"ok": False, "error": "conversation not found"}), 404
        messages = agnes_mgr.list_conversation_messages(conversation_id)
...
def chat_job_detail(job_id: str):
    if not config.agnes_enabled:
        return _disabled_response()
    try:
        job = chat_job_mgr.get(job_id, provider="agnes")
        if job is None:
